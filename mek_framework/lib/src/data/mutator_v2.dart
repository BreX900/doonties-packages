...
final _mutationsProvider = Provider.autoDispose.family((ref, BuildContext context) {
  var mutations = <MutationBloc<Object?, Object?>>[];
  ref.onDispose(() {
    for (final mutation in mutations) {
      unawaited(mutation.close());
    }
    mutations = const [];
  });
  return mutations;
...
extension MutationBlocExtension on WidgetRef {
  MutationBloc<A, R> mutation<A, R>(
    ...
    final sub = listenManual(_mutationsProvider(context), (_, __) {});
    sub.read().add(mutation);
    return mutation;
  }
